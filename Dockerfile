...
#LAMP
RUN apt-get -q -y  --force-yes  install apache2  php php-pear php-mysql
...
RUN service mysql start && mysql -uroot -e "CREATE USER 'root2'@'localhost' IDENTIFIED BY '';  GRANT ALL PRIVILEGES ON *.* TO 'root2'@'localhost' WITH GRANt OPTION; CREATE USER 'root2'@'%' IDENTIFIED BY '' ;  GRANT ALL PRIVILEGES ON *.* TO 'root2'@'%' WITH GRANT OPTION ; FLUSH PRIVILEGES;"
...
ENTRYPOINT service mysql start ; service tor start ; service apache2 restart && cd /opt/proxymity/  &&  while true; do java -cp .:dependency/*:target/com.tools.proxymity-1.0-jar-with-dependencies.jar com.object0r.tools.proxymity.Main > /var/log/proxymity.log 2> /var/log/proxymity.error.log ; killall phantomjs ; sleep 2; done
