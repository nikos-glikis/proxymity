...
ENTRYPOINT service mysql start ; service tor start ; service apache2 restart && cd /opt/proxymity/  &&  while true; do java -cp .:dependency/*:target/com.tools.proxymity-1.0-jar-with-dependencies.jar com.object0r.tools.proxymity.Main > /var/log/proxymity.log 2> /var/log/proxymity.error.log ; sleep 2; done
